...
| POST | `/admin/users` | admin (write) | create an account (`username`, `password`, `role`: `admin` or `viewer`, and an optional `email`). Roles are fixed at creation; changing trust is delete and recreate, which leaves a cleaner trail |
